...
import {Signal} from '@local/signal'
...
console.log('check global[data]', global['data'])
let cliArgDataPath = global['data'] || cliArgv.data || 'data'
let appDataPath = path.join('rigspace', cliArgDataPath)
...
export const setDataPath = value => {
    ...
    appDataPath = path.join('rigspace', cliArgDataPath)
}
...
const getAppDataPath = () => path.join(basePath, 'rigspace', global['data'])
...
if (mainFilePath.indexOf('dist') === -1)                // на папку пакета без dist
    mainFilePath = path.join(process.cwd(), 'dist')

let parentName = mainFilePath.split(path.sep).pop()

while (parentName !== 'dist') {
...
export const ensureDataDirectories = directory => {
    ...
    const appParts = path.join('rigspace', global['data']).split(path.sep)
